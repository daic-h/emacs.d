    ...
    (with-current-buffer buffer
      (unless modeline-git-branch-process
        (let ((process-connection-type nil))
          (setq modeline-git-branch-process (start-process "modeline-git-branch" buffer "git" "symbolic-ref" "HEAD"))
          (set-process-filter   modeline-git-branch-process 'modeline-git-branch-update-modeline)
          (set-process-sentinel modeline-git-branch-process 'modeline-git-branch-clear-process)
          (set-process-query-on-exit-flag modeline-git-branch-process nil))))))

;; プロセスからの出力を整形してモードラインを更新する
(defun modeline-git-branch-update-modeline (process output)
  (when (buffer-live-p (process-buffer process))
    (with-current-buffer (process-buffer process)
      (cond ((string= "fatal: ref HEAD is not a symbolic ref" (substring output 0 -1))
             (setq modeline-git-branch-string "[no-branch]"))
            ((string-match "^fatal" output)
             (setq modeline-git-branch-string "[no-repo]"))
            (t
             (setq modeline-git-branch-string (format "[%s]" (substring output 11 -1)))))
